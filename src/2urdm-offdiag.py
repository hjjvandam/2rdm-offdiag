...
def extract_elements(name,mode,rdm1_a,rdm1_b,rdm2_aa,rdm2_ab,rdm2_bb,mol,xyznm):
    ...
    filename = f"u_{name}_aa_d_{nterms_aa}.dat"
    ...
    filename = f"u_{name}_aa_o_{nterms_aa}.dat"
    with open(filename,mode) as fp:
        for ii in range(norb):
            for jj in range(norb):
                if not (ii == jj):
    ...
    filename = f"u_{name}_ab_d_{nterms_ab}.dat"
    with open(filename,mode) as fp:
        for ii in range(norb):
            for jj in range(norb):
                occ_a = rdm1_a[ii,ii]
    ...
    filename = f"u_{name}_ab_o_{nterms_ab}.dat"
    with open(filename,mode) as fp:
        for ii in range(norb):
            for jj in range(norb):
                if not (ii == jj):
    ...
    filename = f"u_{name}_bb_d_{nterms_bb}.dat"
    with open(filename,mode) as fp:
        for ii in range(norb):
            for jj in range(norb):
                occ_b1 = rdm1_b[ii,ii]
    ...
    filename = f"u_{name}_bb_o_{nterms_bb}.dat"
    with open(filename,mode) as fp:
        for ii in range(norb):
            for jj in range(norb):
                if not (ii == jj):
